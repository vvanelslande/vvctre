...
#include "video_core/renderer/format_reinterpreter.h"
...
OGLTexture RasterizerCache::AllocateSurfaceTexture(const FormatTuple& format_tuple, u32 width,
                                                   u32 height) {
...
RasterizerCache::RasterizerCache() {
    ...
    format_reinterpreter = std::make_unique<FormatReinterpreter>();
...
SurfaceRect_Tuple RasterizerCache::GetSurfaceSubRect(const SurfaceParams& params,
...
Surface RasterizerCache::GetTextureSurface(const Pica::Texture::TextureInfo& info, u32 max_level) {
...
SurfaceSurfaceRect_Tuple RasterizerCache::GetFramebufferSurfaces(
    ...
    for (const auto& set : RangeFromInterval(surface_cache, interval))
        for (const auto& surface : set.second)
            if (surface->pixel_format == PixelFormat::Invalid) {
                LOG_DEBUG(Render_OpenGL, "Surface found with invalid pixel format");
                return true;
            }
    return false;
}
