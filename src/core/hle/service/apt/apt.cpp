...
bool Module::LoadLegacySharedFont() {
    ...
    // generated by the APT:U service. The best way to get is by dumping it from RAM. Citra provides
    // a homebrew app to do this: https://github.com/citra-emu/3dsutils. Put the resulting file
    // "shared_font.bin" in the vvctre "sysdata" directory.
    std::string filepath =
        FileUtil::GetUserPath(FileUtil::UserPath::SysDataDir) + "shared_font.bin";
...
void Module::APTInterface::PrepareToStartApplication(Kernel::HLERequestContext& ctx) {
    ...
    LOG_DEBUG(Service_APT, "called title_id={:#010X} media_type={} flags={:#010X}", title_id,
              media_type, flags);
}
