...
public:
    ...
    MacAddress mac_address; ///< The MAC address of this member.
...
void RoomMember::RoomMemberImpl::MemberLoop() {
    ...
    while (IsConnected()) {
        ...
        ENetEvent event;
        if (enet_host_service(client, &event, 99) > 0) {
            switch (event.type) {
...
void RoomMember::Join(const std::string& nick, const std::string& console_id_hash,
                      const char* server_addr, u16 server_port, const MacAddress& preferred_mac,
                      const std::string& password) {
